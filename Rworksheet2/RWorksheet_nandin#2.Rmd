...
vector
```
Each value in the vector is spaced evenly apart by 0.2 units.
...
```
The full vector x with named elements is displayed in the first output:
"first" is equivalent to the number 3.
"second" is equivalent to the number 0.
"third" is equivalent to the number 9.
The second output shows simply the values linked with the names "first" and "third", which are 3 and 9, respectively. By subsetting the vector according to its names, the given items can be returned as a named vector.
...
names(x)
```
The first output shows the full vector with named elements.
The second output shows only the elements associated with "first" and "third", returning the values 3 and 9.
...
```
All integers from -3 to 2 are included in the initial sequence produced by -3:2: -3, -2, -1, 0, 1, 2.
The second element, which was initially -2, is changed to 0 after x[2] <- 0.
The final vector is -3, 0, -1, 0, 1, 2, with 0 now occupying the second spot in place of -2.
...
Each row represents a team’s rank, with descending ranks from 10 to 20, providing a quick overview of team performance in this segment.
...
```{r} 
vegetables <- c("Squash", "Potato", "Celery", "Tomato", "Cucumber", "Egg Plant", "Lettuce", "Bitter Gourd")
vegetables
```
...
```{r}
vegetables <- c(vegetables, "Ginger", "Bean")
vegetables
```
...
```{r} 
vegetables <- append(vegetables, c("Cabagge", "Green Chili", "Sprout", "Garlic"), after = 5)
length(vegetables)
```
